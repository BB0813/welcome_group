...
class WelcomePlugin(Star):
    ...
    async def _generate_message_with_llm(
        ...
        try:
            ...
            llm_resp = await self.context.llm_generate(
                ...
                prompt=prompt,
                system_prompt=(
                    "你是一个QQ群管理助手，请根据用户的要求生成合适的群消息文案。"
                    "回复要简洁、自然、友好。只返回文案内容，不要添加任何其他解释。"
                ),
            )
    ...
    async def _get_message_template(
        self, event: AstrMessageEvent, message_type: str, fallback: str
    ) -> str:
    ...
    async def on_group_increase(self, event: AstrMessageEvent):
        ...
        try:
            ...
            welcome_template = group_config.get(
            ...
            logger.info(
                f"WelcomePlugin: 准备发送入群欢迎 -> 群 {group_id} 用户 {user_id}"
            )
    ...
    async def on_group_decrease(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if sub_type == "kick_me":
                logger.info(
                    "WelcomePlugin: 机器人被踢出群 "
                    f"{self._get_dict_value(raw, 'group_id')}，"
                    f"操作者: {self._get_dict_value(raw, 'operator_id')}"
                )
                return
            ...
            if sub_type == "leave":
                ...
                template = group_config.get(
                    "leave_message", self.config["default_leave_message"]
                )
                log_label = "退群"
                ...
                template = group_config.get(
                    "kick_message", self.config["default_kick_message"]
                )
                log_label = "被踢"
            ...
            if self.config.get("llm_enabled", False):
                llm_message = await self._generate_message_with_llm(
                    event,
                    (
                        f"请为用户 {user_id} 生成一条简短的{log_label}通知消息，"
                        "要求礼貌。只返回消息内容。"
                    ),
                )
            ...
            logger.info(
                f"WelcomePlugin: 准备发送{log_label}通知 -> "
    ...
    async def set_llm_provider(
        ...
        yield event.plain_result(
            f"已设置 LLM provider 为: {provider_id.strip()}"
        )
    ...
    async def list_llm_providers(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if not providers:
                yield event.plain_result(
                    "没有可用的 LLM provider。请先在 AstrBot 中配置。"
                )
                return
    ...
    def _parse_time(raw: dict) -> str:
        ...
        event_time = WelcomePlugin._get_dict_value(
            raw, "time", time.time()
        )

        try:
            return datetime.fromtimestamp(event_time).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        except Exception:
    ...
    def _to_onebot_message(cls, message_list: list) -> list[dict]:
        ...
        """
        message = [
            cls._component_to_onebot_segment(item)
            for item in message_list
        ]

        # 在真正发送前校验，确保 Plain 等对象已经被转换
    ...
    async def _send_group_msg(
        ...
        try:
            ...
            if bot and hasattr(bot, "send_group_msg"):
                ...
                logger.info(
                    f"WelcomePlugin: 发送消息成功 -> 群 {group_id}"
                )

            elif hasattr(event, "send"):
                # 没有 bot API 时使用 AstrBot 的 event.send
                await event.send(MessageChain(message_list))

                logger.info(
                    f"WelcomePlugin: 发送消息成功(event.send) -> 群 {group_id}"
                )

            else:
                logger.warning(
                    "WelcomePlugin: 无法获取 bot 对象或 send 方法"
                )
    ...
    async def _run_test(
        ...
        template = group_config.get(
            template_field,
            self.config.get(default_key, ""),
        )

        if not template:
            yield event.plain_result(
                "未找到模板，请先通过 /welcome set 等命令配置。"
            )
            return
        ...
        if self.config.get("llm_enabled", False):
            llm_message = await self._generate_message_with_llm(
                event,
                (
                    f"请为用户 {user_id} 生成一条简短的"
                    f"{llm_prompt}消息，要求{llm_tone}。"
                    "只返回消息内容。"
                ),
            )
        ...
        message_list = self._build_onebot_message(
            processed,
            int(user_id),
        )
        ...
        try:
            ...
            logger.error(
                f"WelcomePlugin: 测试{log_label}消息发送失败: {e}"
            )
